    ...
    def after_save_with_oracle_lob() #:nodoc:
      if connection.is_a?(JdbcSpec::Oracle)
        self.class.columns.select { |c| c.sql_type =~ /LOB\(|LOB$/i }.each { |c|
          ...
          unless self.class.composite?
            pk_condition = "#{self.class.primary_key} = #{quote_value(id)}"
          else
            ids = self.class.primary_keys.collect {|p| quote_value(self[p.to_s])}
            pk_condition = [self.class.primary_keys, ids].transpose.collect{|k,v| "#{k} = #{v}"}.join(" AND ")
          end
          connection.write_large_object(c.type == :binary, c.name, self.class.table_name, pk_condition, nil,  value)
        }
      end
...
module ::JdbcSpec
  module ActiveRecordExtensions
    def oracle_connection(config)
      ...
      config[:cursor_sharing] ||= 'similar'
      conn.execute("alter session set nls_date_format = 'YYYY-MM-DD HH24:MI:SS'", "Setting session variable - NLS Date Format")
      conn.execute("alter session set nls_timestamp_format = 'YYYY-MM-DD HH24:MI:SS'", "Setting session variable - NLS Timestamp Format")
      conn.execute("alter session set cursor_sharing = #{config[:cursor_sharing]}", "Setting session variable - Cursor Sharing")
      conn
    end
  end
...
=end
    ...
    def self.extended(object)
      ...
      adapter = object
      adapter.execute("alter session set nls_date_format = 'YYYY-MM-DD HH24:MI:SS'", "Setting session variable - NLS Date Format")
      adapter.execute("alter session set nls_timestamp_format = 'YYYY-MM-DD HH24:MI:SS'", "Setting session variable - NLS Timestamp Format")
      adapter.config[:cursor_sharing] ||= 'similar'
      adapter.execute("alter session set cursor_sharing = #{adapter.config[:cursor_sharing]}", "Setting session variable - Cursor Sharing")
    end

    def jdbc_insert(sql, name = nil, pk = nil, id_value = nil, sequence_name = nil) #:nodoc:
