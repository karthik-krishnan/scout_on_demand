def x_redefine_task(*args, &block)
...
namespace :db do
  x_redefine_task :create => :environment do
  ...
  x_redefine_task :drop => :environment do
  ...
  namespace :structure do
    x_redefine_task :dump => :environment do
  ...
  namespace :test do
    x_redefine_task :clone_structure => [ "db:structure:dump", "db:test:purge" ] do
      abcs = ActiveRecord::Base.configurations
    ...
    x_redefine_task :x_purge => :environment do
      abcs = ActiveRecord::Base.configurations
      ActiveRecord::Base.establish_connection(:test)
      db = ActiveRecord::Base.connection.database_name
