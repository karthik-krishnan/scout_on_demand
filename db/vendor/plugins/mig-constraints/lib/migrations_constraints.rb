...
# This portion has to be re-factor, Since in our app are creating the constraint thro' add_constraint
# and not thro' create table we commented the following block
      ...
      def construct_constraint_clause(type, table_name, column_or_columns, options = {})
        ...
        when :foreign_key
          ...
          if references.is_a?(Hash)
            definition = "FOREIGN KEY (#{quoted_column_names}) REFERENCES #{references[:table]} (#{quote_column_name(references[:column])}) #{references[:referencial_triggered_action]}"
          else
...
#      end
      ...
      def drop_constraint(table_name, options = {})
        ...
        if mysql? 
          execute "ALTER TABLE #{table_name} DROP FOREIGN KEY #{name}" if options[:foreign_key]
          execute "ALTER TABLE #{table_name} DROP INDEX #{name}" if options[:unique]
        else
